...
export async function claimSessionPresenter(sessionId, { presenterId, sessionSecret, authenticatedUserId } = {}) {
  const session = await readSession(sessionId);
  if (!session) {
    return { error: "Session not found", status: 404 };
  }

  // Require authenticated user to match the session creator
  if (!authenticatedUserId || authenticatedUserId !== session.createdBy) {
    return { error: "Forbidden: only the session creator can claim presenter", status: 403 };
  }
...
export async function exchangeRealtimeSubscriptionToken(
  ...
  // Use a short HMAC of the secret instead of the raw sessionSecret
  const channelToken = crypto.createHash('sha256').update(session.sessionSecret).digest('hex').slice(0, 16);
  return {
    realtimeChannel: `collab:${session.id}:${channelToken}`,
  };
}
